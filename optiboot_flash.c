...
// These microcontrollers doesn't have a 4s and 8s WDT setting
...
|| defined(__AVR_ATmega128__) || defined(__AVR_ATmega162__) || defined(__AVR_ATmega169__)      \
|| defined(__AVR_ATmega169P__) || defined(__AVR_ATmega329__) || defined(__AVR_ATmega329P__)    \
|| defined(__AVR_ATmega649__) || defined(__AVR_ATmega649P__) || defined(__AVR_ATmega3290__)    \
|| defined(__AVR_ATmega3290P__) || defined(__AVR_ATmega6490__) || defined(__AVR_ATmega6490P__) \
// Do nothing
#else
...
// endif Assumption above is bad if the reset is in any way dodgy. MarkMLl
...
/* The above has given me enormous trouble. MarkMLl */

  if ((pgm_read_byte_near((void *)0x0000) != 0xff) &&
		((ch & (_BV(BORF) | _BV(EXTRF) | _BV(PORF))) == 0x00))
    appStart(ch);
...
// || defined(BIGBOOT) Removed MarkMLl
        while(len--) {
...
// Simplified version saves enough bytes to allow SP initialisation and better
// appstart() decision. MarkMLl
...
static inline void writebuffer(int8_t memtype, uint8_t *mybuff,
                               uint16_t address, pagelen_t len)
{
  if (memtype == 'E') ; // Error: wait for WDT
        /*
         * Default to writing to Flash program memory.  By making this
         * the default rather than checking for the correct code, we save
         * space on chips that don't support any other memory types.
